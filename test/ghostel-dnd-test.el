...
concatenated."
  (let ((sent ""))
    (cl-letf (((symbol-function 'ghostel--paste-text)
               (lambda (text &optional _require-bracketed)
                 (setq sent (concat sent text))))
...
(ert-deftest ghostel-test-drop-file-pastes-quoted-joined-paths ()
  ...
  (let ((pasted nil)
    ...
    (cl-letf (((symbol-function 'ghostel--paste-text)
               (lambda (text &optional _require-bracketed) (push text pasted)))
...
(ert-deftest ghostel-test-drop-ignores-movement-and-nil-events ()
  ...
  (let ((calls 0))
    (cl-letf (((symbol-function 'ghostel--paste-text)
               (lambda (_text &optional _require-bracketed)
                 (setq calls (1+ calls))))
              ((symbol-function 'ghostel--on-user-input) #'ignore))
