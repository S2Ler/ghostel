...
(defvar-local ghostel--hidden-output nil
  "Non-nil when output found no render window.
Keep this separate from `ghostel--pending-redraw': visible synchronized
output can also remain pending.  Input clears this flag so echo can render
before redisplay notices a newly displayed buffer.")

(defvar-local ghostel--pending-identification nil
  "Non-nil when hidden output changed the mode-line identification.")
...
Known hidden buffers defer mode-line formatting until redisplay."
  (if (and ghostel--hidden-output ghostel-buffer-identification-format)
      ;; `format-spec' kills a temporary buffer, which searches all windows.
      (setq ghostel--pending-identification t)
    (setq ghostel--pending-identification nil)
...
timer.  Hidden output remains pending until the buffer is displayed."
  (if (and (not ghostel--hidden-output)
           (ghostel--get-render-window (current-buffer)))
    ...
    (with-current-buffer buffer
      ...
      (when (and ghostel--term
        ...
        (when-let* ((render-win (ghostel--get-render-window buffer)))
          (setq ghostel--hidden-output nil)
          (when ghostel--pending-identification
            (ghostel--buffer-identification-update))
...
spawn after initialization."
  ...
  (with-current-buffer buffer
    ...
    (setq ghostel--input-mode 'semi-char
          ...
          ghostel--hidden-output nil
          ghostel--pending-identification nil
